...
const tokens: { [key: string]: string } = {
  "0x570a5d26f7765ecb712c0924e4de545b89fd43df": "SOLANA (SOL)",
...
const pairs: Array<string> = TokenPermutation();

function App() {

  const [tokenpairs, setTokenPairs] = useState(Array<Array<string>>);

  useEffect(() => {

    const newTokenPair: Array<Array<string>> = []

    for (let i = 0; i < pairs.length; i++) {

      const [token0, token1] = GetTokensByUniqueName(pairs[i])

      newTokenPair.push([token0, token1])
      ...
      <ctx.Provider value={{
        ...
        <div className="cards_view">
          {tokenpairs.map((pp, idx) => (
            <div className={"pool_card"} key={pp.join("")}>
              <PoolPairView
                tokens_addr={tokenpairs[idx]} />
            </div>
...
function GetTokensByUniqueName(name: string): Array<string> {
  return name.split("-")
}
export default App
